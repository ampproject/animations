 ...
 */
export function offset(offset, root = document) {
...
function getAllDocs() {
    const allShadowRoots = getAllElements(document)
        .map(getShadowRoot)
        .filter(sr => sr);
    return allShadowRoots.concat(document);
}
/**
 ...
 */
const existingRulesMap = new WeakMap();
/**
 ...
 */
const creatseAnimationPauseStyle = function () {
 ...
 * @param callback
 */
function after(obj, prop, callback) {
    if (!(prop in obj)) {
        return;
    }
    const old = obj[prop];
 ...
 */
function attachStyleElement(root) {
    if ('host' in root) {
 ...
 */
function getAttrName(pseudoElt) {
 ...
 */
function getDelayString(el, pseudoElt, offset) {
 ...
 * @param pseudoElt The pseudo element selector.
 */
function applyDelay(el, offset, style, pseudoElt) {
 ...
 * @param offset The animation offset, in milliseconds.
 */
function applyOffsets(el, offset) {
...
// created.
captureShadowRoots();
//# sourceMappingURL=animation-test-controller.js.map